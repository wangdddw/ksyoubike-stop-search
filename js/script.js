...
function getBikeStopList(){
  ...
  for (let i = 0; i < bikeStop.length; i++) {
    ...
    markers.addLayer(L.marker([bikeStop[i].lat, bikeStop[i].lng], { icon: bikeIcon }).unbindPopup().bindPopup(`<span>${bikeStop[i].sna.substring(11, 30)}</span><br/><span>可借：${bikeStop[i].sbi}</span><span>可停：${bikeStop[i].bemp}</span>`).openPopup());
  };
...
function updateData(e){
  ...
  L.marker(location,{ icon: bikeIcon }).addTo(map).unbindPopup().bindPopup(`<span>${tempName}</span><br/><span>可借：${sbi}</span><span>可停：${bemp}</span>`).openPopup();
};
